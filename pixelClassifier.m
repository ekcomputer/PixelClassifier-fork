% TODO: clear branch for no block proc bc whole thing can be written as
% fxn, if I want...
% Note "if 1==1 switch to avoid using blockproc"
clear
% clc
blockProcessing_pld = false; % depricated in favor of new blockproc scheme!
...
for imIndex = 1:length(imagePaths)
    try % for fault tolerance so following images will still run even if an error...
        ...
        if env.blockProcessing
            stepTime=tic;
            mapinfo=geotiffinfo(imagePaths{imIndex});
            env_copy=env; % make copy to avoid calling global var in parallel blockproc
                ...
                if ismember(band, env.radar_bands) % for radar bands
                    if blockProcessing_old == true
                        F = imageFeatures(I(:,:,band),model.sigmas,...
                            ...
                            [],[]);
                    else % blockProcessing_old == False
    ...
    %         load(F_obj_pth); % returns F ( 3D image)
        ...
        %         end
            ...
            %% Proceed to write output% HERE continue blockproc output
            [fpath,fname] = fileparts(imagePaths{imIndex});
            if blockProcessing_old == true
                ...
                if 1 == 1 %env.useFullExtentClassifier==false % Depricated: need to use blockproc on input tif to avoid mem error...
